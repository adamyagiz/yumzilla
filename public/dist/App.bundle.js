...
function autocomplete(input, latInput, lngInput) {
  ...
  dropdown.addListener('place_changed', function () {
    // google maps way to addEventListener
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 37.84;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -122.25;

  _axios2.default.get('/api/v1/stores/near/' + lat + '/' + lng).then(function (res) {
    var places = res.data;
    if (!places.length) {
      alert('no places');
      return;
...
function searchResultsHTML(stores) {
  return stores.map(function (store) {
    return '<a href="/store/' + store.slug + '" class="search__result"><store>' + store.name + '</store></a>';
  }).join('');
}
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/v1/search/' + this.value).then(function (res) {
      if (res.data.length) {
